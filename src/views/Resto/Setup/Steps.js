...
const Steps = () => {
  ...
  const updateAsyncStorage = async ({
    ...
    const savePromise = AsyncStorage.setItem("restoSetupProfile", data)
  ...
  const onSubmit = async () => {
    ...
    apiData.append("image", {
      ...
      name: "restoCoverPhoto.jpg",
      type: "image/jpeg",
    })
    let apiPromise = axios.post("uploadimage/resto", apiData)
    ...
    const sendData = {
      ...
      restoCategory: dataProfile.category,
    ...
    for (let i = 0; i < dataWorkHour.length; i++) {
      const schedulePromise = axios.post(
        "restooperation/add",
        {
          ...dataWorkHour[i],
          restoId: restoId,
          openResto: dataWorkHour[i]?.openResto || "00:00",
          closeResto: dataWorkHour[i]?.closeResto || "00:00",
    ...
    const resetPromise = AsyncStorage.removeItem("restoSetupProfile")
  ...
  const restoreFunc = async () => {
    const restorePromise = AsyncStorage.getItem("restoSetupProfile")
